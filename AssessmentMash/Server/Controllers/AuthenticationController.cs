...
namespace AssessmentMash.Server.Controllers
{
    ...
    public class AuthenticationController : ControllerBase
    {
        ...
        public async Task<IActionResult> RegisterUser(RegisterModel model)
        {
            ...
            else if (model.Email!.ToLower().StartsWith("seller"))
            {
                var chkRole = await appDbContext.UserRoles.FirstOrDefaultAsync(_ => _.RoleName!.ToLower().Equals("seller"));
                if (chkRole is null)
                    role = "Seller";
                else
                    return BadRequest("Seller already registered");
            }
            else
            {
                role = "Buyer";
            ...
            var chk = await appDbContext.Registers.FirstOrDefaultAsync(_ => _.Email!.ToLower().Equals(model.Email!.ToLower()));
            if (chk is not null) return BadRequest(-1);

            var entity = appDbContext.Registers.Add(
                new Register()
                {
                    Email = model.Email,
                    Name = model.Name,
        ...
        {
            var users = await appDbContext.Registers.ToListAsync();
        ...
        {
            var user = await appDbContext.Registers.FirstOrDefaultAsync(_ => _.Email.ToLower().Equals(email.ToLower()));
